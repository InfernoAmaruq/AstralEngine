...
static int llex (LexState *ls, SemInfo *seminfo) {
  ...
  for (;;) {
	...
	if (ls->current == '*') {
      ...
      case '/': {
	next(ls);
	if (ls->current == '=') {next(ls); return TK_DIVEQ;}
	else return '/';
      }
      case '%': {
	next(ls);
	...
	if (ls->current == '^') {next(ls);
		if (ls->current == '=') {next(ls); return TK_XOREQ;}
		else return TK_XOR;
	}
	if (ls->current == '=') {next(ls); return TK_POWEQ;}
	else return '^';
      }
      case '-': {
        next(ls);
	...
	if (ls->current == '<') {next(ls); 
      ...
      case '~': {
        next(ls);
	if (ls->current == '~') {next(ls); return TK_DEC;};
        if (ls->current != '=') return '~';
	...
	else if (ls->current == '=') {next(ls); return TK_BANDEQ;}
	else return '&';
      }
      case '"':
      case '\'': {
