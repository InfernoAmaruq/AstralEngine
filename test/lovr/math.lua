group('math', function()
  ...
  group('quaternion', function()
    test('angleaxis', function()
      expect({ quaternion(1.2, 1, 0, 0):unpack() }).to.equal({ .564642, 0, 0, 0.825336 }, 1e-4)
  ...
  test('gammaToLinear', function()
    ...
    expect({ lovr.math.gammaToLinear(.5, .5, .5) }).to.equal({ .214, .214, .214 }, 1e-3)
    expect({ lovr.math.gammaToLinear(.5, .5, .5, .5) }).to.equal({ .214, .214, .214, .5 }, 1e-3)
    expect({ lovr.math.gammaToLinear(.5, .5, .5, .5, 7) }).to.equal({ .214, .214, .214, .5 }, 1e-3)
    expect({ lovr.math.gammaToLinear({ .5, .5, .5, .5 }) }).to.equal({ .214, .214, .214, .5 }, 1e-3)
    expect({ lovr.math.gammaToLinear({ .5, .5, .5 }) }).to.equal({ .214, .214, .214 }, 1e-3)
    expect(lovr.math.gammaToLinear(.5)).to.equal(.214, 1e-3)
  end)

  test('linearToGamma', function()
    expect(function() lovr.math.linearToGamma() end).to.fail()
    expect({ lovr.math.linearToGamma(.5, .5, .5) }).to.equal({ .735, .735, .735 }, 1e-3)
    expect({ lovr.math.linearToGamma(.5, .5, .5, .5) }).to.equal({ .735, .735, .735, .5 }, 1e-3)
    expect({ lovr.math.linearToGamma(.5, .5, .5, .5, 7) }).to.equal({ .735, .735, .735, .5 }, 1e-3)
    expect({ lovr.math.linearToGamma({ .5, .5, .5, .5 }) }).to.equal({ .735, .735, .735, .5 }, 1e-3)
    expect({ lovr.math.linearToGamma({ .5, .5, .5 }) }).to.equal({ .735, .735, .735 }, 1e-3)
    expect(lovr.math.linearToGamma(.5)).to.equal(.735, 1e-3)
  end)
end)
