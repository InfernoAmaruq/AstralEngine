...
local GETTERS = {
    Static = function(self)
        return self.World:IsStaticTag(self.ColRef:getTag())
    end,
...
function mt.__create(DATA, e)
    ...
    local World = DATA.World or Physics.GetMainWorld()

    AstralEngine.Assert(World, "No world provided or no main world created!","COLLIDER")

    local RawWorld = World.LovrWorld
    local COLLIDER = RawWorld:newCollider(0,0,0)
    COLLIDER:setPose(Transform.Position,Transform.Orientation)
    COLLIDER:setTag(DATA.Tag or World.DefaultTag)
    ...
    local IsStatic = World:IsStaticTag(COLLIDER:getTag())
    ...
    Physics.__BindCollider(World, T, Transform)
