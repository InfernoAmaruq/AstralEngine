...
local Index = {
    ...
    Disable = function()
        ...
        local Obj = 0
        for i in pairs(WaitList) do
            Obj = Obj + 1
            AstralEngine.Log("GetService wait list still has: " .. i, "info", "SERVICE MANAGER")
        end

        if Obj > 0 then
            AstralEngine.Error("GetService fatal error! Service wait list not fulfilled!", "SERVICE MANAGER", 2)
        end
...
local function ProcessNonexistantService(n, repair)
    WaitList[n] = true
                    ...
                    while true do
                        ...
                        if not k or k == repair then
                            if k == repair then
                                ...
                                AstralEngine.Log("REQUIRE REPAIR SUCCESSFUL FOR: " .. k, "info", "SERVICE MANAGER")
                            end
                            break
                        end
                        j = j + 1
...
local f = setmetatable({}, {
    __call = function(_, n, repair)
        local s = Services[n]
        if not s and Active then
            s = ProcessNonexistantService(n, repair)
            WaitList[n] = nil
        elseif not s and not Active then
            AstralEngine.Error("Attempt to index non-existant service: " .. n, "SERVICE MANAGER", 1)
        end
        return s
    end,
    __index = Index,
