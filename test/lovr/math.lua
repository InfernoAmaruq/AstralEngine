group('math', function()
  ...
  group('mat4', function()
    ...
    test('set Position/Scale/Orientation/Pose', function()
      ...
      expect({ m:unpack() }).to.equal({ 0,0,0, 1.5,2.5,3.5, 0,0,0,0 }, 1e-4)
      m:setScale({ 3.5, 4.5, 5.5 })
      expect({ m:unpack() }).to.equal({ 0,0,0, 3.5,4.5,5.5, 0,0,0,0 }, 1e-4)

      m:setPosition(1, 2, 3)
      expect({ m:unpack() }).to.equal({ 1,2,3, 3.5,4.5,5.5, 0,0,0,0 }, 1e-4)
      m:setPosition({ 3, 4, 5 })
      expect({ m:unpack() }).to.equal({ 3,4,5, 3.5,4.5,5.5, 0,0,0,0 }, 1e-4)

      m:setOrientation(1, 0, 1, 0)
      expect({ m:unpack() }).to.equal({ 3,4,5, 3.5,4.5,5.5, 1,0,1,0 }, 1e-4)
      m:setOrientation(quaternion.angleaxis(2, 1, 0, 0))
      expect({ m:unpack() }).to.equal({ 3,4,5, 3.5,4.5,5.5, 2,1,0,0 }, 1e-4)

      m:setPose(7,8,9, 1,0,0,1)
      expect({ m:unpack() }).to.equal({ 7,8,9, 3.5,4.5,5.5, 1,0,0,1 }, 1e-4)
      m:setPose({ 2, 5, 7 }, quaternion(3, 1, 0, 0))
      expect({ m:unpack() }).to.equal({ 2,5,7, 3.5,4.5,5.5, 3,1,0,0 }, 1e-4)
    end)
  end)
end)
