...
local CurLinkerMode = nil
local function LinkerGsub(Word)
    return LinkerSubstitution[CurLinkerMode][Word] or Word
end

local function Compile(Tab,Tag)
    RETURN_DEFAULT(Tab,DEFAULTS[Tag])

    local Defined = Tab.Define
    Tab.Define = nil
    ...
    return Out.."\n//LINKED\n"..BodyStart..LinkedBody:gsub("%f[%w_][%w_]+%f[^%w_]",LinkerGsub)..BodyEnd
end
...
local FileEndings = {"",".glsl"}
local GETINCLUDE_TEMP = table.new(3,0)
local ShaderFolder = unpack(lovr.filesystem.getAliased("Shaders"))

local function GetIncluded(Into,IncludeTable)
    local CalledFrom = lovr.filesystem.folderFromPath(lovr.filesystem.getCurrentPath(3) or "")
    for _,v in pairs(IncludeTable) do
        GETINCLUDE_TEMP[1] = ShaderFolder.."/"..v
        GETINCLUDE_TEMP[2] = CalledFrom.."/"..v
        GETINCLUDE_TEMP[3] = v
        local Out = false
        for _,p in ipairs(GETINCLUDE_TEMP) do
            for _,ext in ipairs(FileEndings) do
                local FilePath = lovr.filesystem.normalize(p..ext)
                if lovr.filesystem.isFile(FilePath) then
                    Out = true

                    Into[#Into + 1] = FilePath

                    break
                elseif lovr.filesystem.isDirectory(FilePath) then
                    Out = true

                    for _,v in ipairs(lovr.filesystem.getDirectoryItems(FilePath)) do
                        Into[#Into + 1] = FilePath.."/"..v
                    end

                    break
                end
            end
            if Out then Out = false break end
        end
    end
end
...
function ShaderService.ComposeShader(Type,Search,Params)
    ...
    local SearchIn = lovr.filesystem.getAliasedFiles("Shaders/"..Search)

    if Params and Params.Include then
        GetIncluded(SearchIn,Params.Include)
    end

    for _,a in ipairs(SearchIn) do
        if FileSystem.isFile(a) then
            ...
            if Key == &ST_Graphics then
                ...
                Buckets[&ST_Vertex][#Buckets[&ST_Vertex]+1] = Pragmas
                Buckets[&ST_Fragment][#Buckets[&ST_Fragment]+1] = TEXT
                Buckets[&ST_Fragment][#Buckets[&ST_Fragment]+1] = Pragmas
            else
    ...
    for i in pairs(Buckets) do
        if bit.band(i, Type) ~= Type then Buckets[i] = nil end
    end

    if Type == &ST_Graphics then
        Buckets[&ST_Vertex].Define = Params.Define and Params.Define.Vertex
        Buckets[&ST_Fragment].Define = Params.Define and Params.Define.Fragment
        return CompileGraphics(Buckets)
    elseif Type == &ST_Fragment then
        Buckets[&ST_Fragment].Define = Params.Define and Params.Define.Fragment or Params.Define
        return Compile(Buckets[&ST_Fragment], &ST_Fragment)
    elseif Type == &ST_Vertex then
        Buckets[&ST_Vertex].Define = Params.Define and Params.Define.Vertex or Params.Define
        return Compile(Buckets[&ST_Vertex], &ST_Vertex)
    else
        Buckets[&ST_Compute].Define = Params.Define and Params.Define.Compute or Params.Define
